 ...
 * here. This jpalamutscript will grab settings from customizer controls, and
...
( function( $ ) {
	...
	wp.customize( 'custom_logo_max_width', function( value ) {
		value.bind( function( newval ) {
			$( '.site-logo img' ).css( 'height', newval );
		} );
	} );
  
  wp.customize( 'show_bottombar', function( value ) {
    value.bind( function( to ) {
      if ( true === to ) {
	...
	wp.customize( 'powered_by_palamut', function( value ) {
		value.bind( function( to ) {
			if ( true === to ) {
				$( '.powered-by-palamut' ).removeClass( 'hidden' );
			} else {
				$( '.powered-by-palamut' ).addClass( 'hidden' );
			}
		} );
	} );
