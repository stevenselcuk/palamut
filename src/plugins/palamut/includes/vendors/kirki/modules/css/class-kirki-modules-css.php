...
class Kirki_Modules_CSS {
	...
	public function init() {
		...
		// @TODO : This is hardcore if... holy shit.
		if ( class_exists( 'Classic_Editor' ) || version_compare( $GLOBALS['wp_version'], '5.0', '<' ) && in_array( $typenow, array( 'post', 'page' ), true ) ) {

			add_action( 'admin_init', array( $this, 'old_editor_enqueue_styles' ) );
		}
	 ...
	 * @return void
	 */
	public function old_editor_enqueue_styles() {

		$args = array(
			'action' => apply_filters( 'kirki_styles_action_handle', 'kirki-styles' ),
		);

		add_editor_style( add_query_arg( $args, site_url() ) );
	}
