...
const blockAttributes = {
	title: {
		type: 'string',
		selector: '.wp-block-palamut-accordion__title',
	},
	open: {
		type: 'boolean',
		default: false,
...
const settings = {
	...
	save( { attributes } ) {
		...
		const backgroundClasses = classnames(
			'wp-block-palamut-accordion-item',
			open ? 'wp-block-palamut-accordion-item--open' : null, {}
		);

		const titleClasses = classnames(
			'wp-block-palamut-accordion-item__title', {
				...
				{ ! RichText.isEmpty( title ) &&
					<details open={ open }>
						<RichText.Content
							...
							style={ titleStyles }
						/>
						<div className="wp-block-palamut-accordion-item__content" style={ borderStyle }>
							<InnerBlocks.Content />
						</div>
					</details>
				}
