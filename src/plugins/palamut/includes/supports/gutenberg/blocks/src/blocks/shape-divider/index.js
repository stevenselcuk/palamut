...
const settings = {
	...
	transforms: {
		from: [
			...
			},
			{
				type: 'block',
				blocks: [ 'core/spacer' ],
				transform: ( { height } ) => createBlock( `palamut/${ name }`, {
					height: height,
				} ),
			},
			{
				type: 'block',
				blocks: [ 'palamut/dynamic-separator' ],
				transform: ( { height } ) => createBlock( `palamut/${ name }`, {
					height: height,
				} ),
			},
			{
				...
				transform: () => createBlock( `palamut/${ name }` ),
			},
		],
	},
	...
	save( { attributes, className } ) {
		...
		const classes = classnames(
			className, {
			[ `palamut-shape-divider-${ palamut.id }` ] : palamut && ( typeof palamut.id != 'undefined' ),
		...
		return (
			<div className={ classes } style={ styles } aria-hidden="true">
				<div className="wp-block-palamut-shape-divider__svg-wrapper" style={ { height: shapeHeight } }>
					{ getDividerFromStyle( attributes.className ) }
				</div>
				<div className="wp-block-palamut-shape-divider__alt-wrapper" style={ { height: backgroundHeight } }></div>
			</div>
		);
	},
};
