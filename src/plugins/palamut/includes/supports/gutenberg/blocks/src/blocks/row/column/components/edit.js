	...
	render() {
		...
		const classes = classnames(
			'wp-block-palamut-column', {
				[ `palamut-column-${ palamut.id }` ] : palamut && ( typeof palamut.id != 'undefined' ),
				'wp-block-palamut-column-placeholder' : columnBlocks && columnBlocks.innerBlocks && Object.keys( columnBlocks.innerBlocks ).length < 1,
			}
		);
			...
			return [
				<Fragment>
					...
					<div
						...
						>
						<div className="wp-block-palamut-column">
		...
		return [
			<Fragment>
				...
				<span className={ classnames( 'palamut-resizeable-width', { 'is-resizing' : this.state.resizing } ) }>
				...
				<ResizableBox
					...
					onResize={ ( event, direction, elt, delta ) => {
						...
						let parentBlockClientRect 	= document.getElementById( 'block-' + parentId ).getElementsByClassName( 'wp-block-palamut-row__inner' )[0].getBoundingClientRect();
						...
						document.getElementById( 'block-' + this.props.clientId ).getElementsByClassName( 'wp-block-palamut-column' )[0].style.width = 'auto';
