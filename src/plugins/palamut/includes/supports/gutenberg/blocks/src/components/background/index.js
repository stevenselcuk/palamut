...
export const BLOCKS_WITH_AUTOPADDING = [ 'palamut/row', 'palamut/column', 'palamut/media-card', 'palamut/features', 'palamut/feature' ];
		...
		return(
			<Fragment>
				<PanelBody
					...
					initialOpen={ false }
					className="components-panel__body--palamut-background-panel"
				>
					...
					{ ! hasParallax && FocalPointPicker && backgroundType == 'image' && backgroundRepeat !== 'repeat' && (
						<FocalPointPicker
							...
							className="components-focal-point-picker--palamut"
						/>
					) }
					{ overlaySelect() }
					...
					{ backgroundType == 'image' && (
						<SelectControl
							label={ __( 'Repeat' ) }
							className="components-background-display-select--palamut"
					...
					<Button
						className="components-button--palamut-remove-background-image"
						type="button"
						isDefault
