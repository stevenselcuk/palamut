...
function FontFamilyPicker( { label, value, help, instanceId, onChange, className, ...props } ) {
	const id = `inspector-palamut-font-family-${ instanceId }`;
	const fonts = [
	...
	const onChangeValue = ( event ) => {
		...
		let ba 	 		= '';
		const googleFonts_attr = ':100,100italic,200,200italic,300,300italic,400,400italic,500,500italic,600,600italic,700,700italic,800,800italic,900,900italic';
		const link 		= document.createElement( 'link' );
	    link.rel 		= 'stylesheet';

		if ( typeof meta !== 'undefined' && typeof meta._palamut_attr !== 'undefined' ) {
			ba = meta._palamut_attr;
		}
	...
	return (
		<BaseControl label={ label } id={ id } help={ help } className={ className }>
			<select
				id={ id }
				className="components-select-control__input components-select-control__input--palamut-fontfamily"
